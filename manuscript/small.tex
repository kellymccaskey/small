...
For the worst-case scenario ($\beta_{cons} = -1$, $k = 9$, and $N = 30$), the ML estimate has an upward bias of about 69\%, while the PML estimate has an upward bias of only about 6\%.\footnote{The online appendix shows the expected value and (absolute) bias of these estimates.}
...
Even for a larger sample of $N = 210$, the variance of the ML estimator is about 6\%, 10\%, and 15\% larger than the PML estimator.\footnote{The online appendix shows the variance inflation $= 100 \times \left(\frac{Var(\hat{\beta} ^{mle})}{Var(\hat{\beta}^{pmle})} - 1\right)$.}
