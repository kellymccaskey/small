...
simulate_pars <- function() {
  ...
  n <- 200000
  # while (n <  | n > 200000) {
  #   n <- round(abs(rcauchy(1, location = 0, scale = 5000))) + 2000  # number of observations
  # }; n
  
  b_cons <- runif(1, -12, -4)  # value of the interecept
...
system.time({
  ...
  df <- foreach (i = 1:n_dgps, 
                 ...
                 .packages = c("clusterGeneration", "brglm")) %dopar% {
                   write(paste("\n...\nWorking on DGP ", i, " of ", n_dgps, ". Random draw is ", runif(1), ". Sample size is ", pars$n, ".", sep = ""), file = "output.txt", append = TRUE)
                   simulate(n_sims = 500)
...
saveRDS(df, file = "R/simulations/sample-size-simulations-expts.RData")
